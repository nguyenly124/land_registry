...
export default function Profile() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const idNumber = auth.CCCD ?? userInfo.idNumber;
        const address = auth.address ?? userInfo.address ?? "";
        const phone = auth.phone ?? userInfo.phone ?? "";
        const gmail = auth.gmail ?? userInfo.gmail ?? "";
  ...
  // Chỉ tính thay đổi trên thông tin (không tính ảnh)
  useEffect(() => {
    setHasChanges(JSON.stringify(userInfo) !== JSON.stringify(originalInfo));
  }, [userInfo, originalInfo]);
  ...
  const handleEditToggle = () => {
    if (isEditing) {
      // Hủy => reset về originalInfo và ảnh theo authUser
      setUserInfo({ ...originalInfo });
      try {
        const raw = localStorage.getItem("authUser");
        if (raw) {
  ...
  const handleSave = () => {
    ...
    if (userInfo.gmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userInfo.gmail)) {
      alert("Email không hợp lệ.");
      return;
    }
    if (userInfo.phone && !/^[0-9+\-\s()]{8,20}$/.test(userInfo.phone)) {
      alert("Số điện thoại không hợp lệ.");
      return;
    }

    // Lưu ảnh vào map demo
    try {
      updateUserCCCD(userInfo.idNumber, frontPreview ?? undefined, backPreview ?? undefined);
    } catch {
      // ignore
    ...
    // Cập nhật authUser trong localStorage
    try {
      const raw = localStorage.getItem("authUser");
      if (raw) {
        const auth = JSON.parse(raw) as Partial<ExtendedAuth>;
    ...
    // Đồng bộ vào các list demo (nếu có)
    try {
      const lists = [userAccounts as ExtendedAuth[], adminAccounts as ExtendedAuth[]];
      for (const list of lists) {
        const idx = list.findIndex(
          (u) => u.CCCD === originalInfo.idNumber || u.username === originalInfo.fullName
        );
        if (idx !== -1) {
          list[idx].username = userInfo.fullName;
          list[idx].CCCD = userInfo.idNumber;
          list[idx].birthDate = userInfo.birthDate;
          list[idx].address = userInfo.address;
          list[idx].phone = userInfo.phone;
          list[idx].gmail = userInfo.gmail;
          if (frontPreview) list[idx].cccdFront = frontPreview;
          if (backPreview) list[idx].cccdBack = backPreview;
          break;
        }
      ...
      <div className="flex gap-12 w-full">
        ...
        <div className="w-1/2">
          ...
          <div className="flex flex-col gap-8 my-8">
            ...
            <div className="flex items-center">
              <label className="font-semibold min-w-[120px]">Số điện thoại: </label>
              {isEditing ? (
                <input
                  type="text"
            ...
            <div className="flex items-center">
              <label className="font-semibold min-w-[120px]">Gmail: </label>
              {isEditing ? (
                <input
                  type="text"
                ...
                <span className="ml-2">{userInfo.gmail || "--"}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex my-5 gap-10 justify-center">
            ...
            {/* NÚT LƯU: chỉ enable khi có chỉnh sửa THÔNG TIN (hasChanges) */}
            <button
        ...
        {/* Cột upload ảnh */}
        <div className="flex flex-col w-1/2 gap-2 ml-5 mt-32">
          <label className="font-semibold">Ảnh CCCD:</label>
          ...
          >
            {frontPreview ? (
              ...
              <div className="text-center text-gray-400">
                {isEditing ? (
                  "Click để tải ảnh Mặt trước"
                ) : (
                  <>
                    <img src="/image/imagePlus.png" alt="placeholder" className="w-20 h-20 mx-auto" />
                    <div className="mt-2">Không có ảnh</div>
                  </>
                )}
              </div>
            )}
          ...
          {/* Lưu ý: upload ảnh không kích hoạt nút Lưu (theo yêu cầu) */}
        </div>
      </div>
    </div>
  );
